...
//var regexATTR_simple = "(?:(?:\\[\\s*[0-9][0-9]*\\s*\\])"+XMLNAME+"|"+XMLNAME+"(?:\\[\\s*[0-9][0-9]*\\s*\\])?)";
//var regexATTR = "(?:(?:\\[\\s*[0-9][0-9]*\\s*\\])"+XMLNAME+"|"+XMLNAME+"(?:\\[\\s*[0-9][0-9]*\\s*\\])?)";
           if(null == this.$keywords)this.$keywords={};
//console.log(this.createKeywordMapper({tok:a}));           
//this.$keywords[a]=tok;
           var attr = "(?:(?:\\[\\s*[0-9][0-9]*\\s*\\])"+a+"|"+a+"(?:\\[\\s*[0-9][0-9]*\\s*\\])?)"
   ...
   var r_keywords = '(?:\\b(?:for|give|distinct|sort|by|desc|asc|filter|where|over|all)\\b)';
   var r_functions = '(?:(?:descendants|lbrothers|rbrothers|sons|depth_first_order|order_span_min|order_span_max|depth|lower|upper|length|substr|tr|replace|substitute|match|ceil|floor|round|trunc|percnt|name|type_of|id|file|tree_no|address|abs|exp|power|log|sqrt|ln)\\b)';
   var r_agreg_functions = '(?:(?:min|max|sum|avg|count|ratio|concat|row_number|rank|dense_rank)\\b)';
