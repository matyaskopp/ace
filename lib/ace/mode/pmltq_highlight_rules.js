...
function _push_next(currentState, stack) { // the last item in this._push array is on the top of stack
    if(this._push.constructor === Array) {
      for(var i in this._push)stack.unshift(this._push[i]);
    } else {
      stack.unshift(this._push);
    }
//    alert("_push_next: "+currentState+"\nSTACK:\n\t"+stack.join("\n\t"));
    return this._next;
};
function _pop(currentState, stack) {
//    alert("_pop: "+currentState+"\nSTACK:\n\t"+stack.join("\n\t"));
    return stack.shift() || "ERROR";
};
...
var PmltqHighlightRules = function() {
    ...
    var test = [{ /*ref*/
            ...
            { /*simple_test->predicate*/
                ...
                regex: "(\\s*)("+TYPE_PREFIX+")\\b(\\s*)("+regexATTR+")",
                _push: "simple_test",
                _next: "step",
                next: _push_next
            },
            { /*term -> SIMPLE ATTRIBUTE*/
                token: ["SP","ATTRIBUTE.D.X"],
                regex: "(\\s*)("+regexATTR+")",
                _push: "simple_test",
                _next: "step",
                next: _push_next
